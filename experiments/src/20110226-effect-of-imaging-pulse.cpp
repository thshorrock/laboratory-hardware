...
main  (int ac, char **av)
{
  ...
  ICR::directory dir("/data/2011/02/28/water/var_gain");  //the current directory
  ...
  //scope.auto_calibrate(false); //turn calibration off
   ...
   //scope.set_coupling(location::C4, coupling::D50);
	...
	lecroy_volts_per_div = 400e-3;
      ...
      subdir = "/first_few_at_new_pressure";
      dir.mkdir(dir.get_directory()+subdir); //create the subdirectory
      dir.pushd(dir.get_directory()+subdir); //goto subdirectory
//create a readme file in the directory.
      ...
      scope.volts_per_div(location::C3, 3.*lecroy_volts_per_div);
      //image the first 20
      for(size_t j=0;j<20;++j){
	readme README_EXP_2(dir.get_directory());
	README_EXP_2  ("Lecroy volts per div", lecroy_volts_per_div);
	aline a = get_aline(driving_pulse, scope);
	std::string index = stringify_with_zeros(j,2); //pad the index with zeros
      
	ICR::file Filename_GP(dir,"driving_wave_only_new_pressure_"+index+"_GP.dat");
	...
	("-----------------------")   
	;
      //skip the next 5 (let it stabalise)
      for(size_t j =0; j<30; ++j){
	driving_pulse.trigger_now();
	++pd_imag;
      }
      //save the next 5 to give indication of reproducability
      for(size_t j=0;j<5;++j){
	driving_pulse.trigger_now();
	lecroy_file file =  scope.get_waveform(location::C3);
	aline a = file.get_data1();
	std::string index = stringify_with_zeros(j,2); //pad the index with zeros
      
	ICR::file Filename_GP(dir,"driving_wave_only_"+index+"_GP.dat");
	...
	++pd_imag;
      ...
      //start the average
      driving_pulse.trigger_now();
      aline average =  scope.get_waveform(location::C3).get_data1();
      ++pd_imag;
      const size_t repeats = 40;
      for(size_t j=0;j<(repeats-1);++j){
	driving_pulse.trigger_now();
	lecroy_file file =  scope.get_waveform(location::C3);
	average += file.get_data1();
	++pd_imag;
      }
      average/=repeats;
      ICR::file Filename_average_GP(dir,"driving_wave_only_averaged_by_"+stringify(repeats)+"_GP.dat");
      //save gnuplot
      ICR::lecroy::save_gnuplot(average,   Filename_average_GP.full());
      
      //save another 5 to check reproducability
